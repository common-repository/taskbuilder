...
$wppm_proj_comment = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}wppm_project_comment where proj_id='".$project_id."' ORDER BY $orderby_sql LIMIT 1;");
$attachments = array();
...
foreach($matches as $match){
	switch($match){
		...
		case '{proj_comment_body}':
			$flag= true;
			if(!empty($this->get_proj_comment_body($project_id))){
