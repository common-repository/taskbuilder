...
if($current_user->has_cap('manage_options')  || $wppm_current_user_capability == 'wppm_admin'){
  if(!empty($proj_attr )){
    $where = " where project_name = '".$proj_attr."' Group by proj.id ORDER BY ".$orderby_proj_sql;
  }else{
    $where = " Group by proj.id ORDER BY ".$orderby_proj_sql;
  }
}else{
  $where =  " where ((FIND_IN_SET($current_user->ID,proj.users)) OR (proj.id = proj_meta.project_id AND proj_meta.meta_key='public_project' AND proj_meta.meta_value=1) OR ( proj.created_by='$current_user->ID')) $wppm_proj_attr Group by proj.id ORDER BY ".$orderby_proj_sql;
}
...
<form id="wppm_task_list_frm">
  <div id="wppm_task_list_container">
    ...
    <div class="row">
      <div class="col-sm-6">
        <div class="wppm-bulk-actions" style=" margin-left: -14px !important;">
            <button 
              ...
              <?php esc_attr_e( 'Bulk Actions ', 'taskbuilder' ); ?><img src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/wppm_chevron_down_icon.svg'); ?>" alt="chevron_down">
            </button>
            <div id="wppm-bulk-actions" class="gpopover wppm-popover-menu wppm-proj-bulk-actions">
              <div class="wppm-popover-menu-item" onclick="wppm_bulk_change_task_status('<?php echo esc_attr( wp_create_nonce( 'wppm_bulk_change_task_status' ) ); ?>');">
                <span><img style="margin-right:5px;" src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/gps-navigation.svg'); ?>" alt="task_status"><?php echo esc_html_e('Change Status','taskbuilder'); ?></span>
              </div>
              <div class="wppm-popover-menu-item" onclick="wppm_bulk_change_task_priority('<?php echo esc_attr( wp_create_nonce( 'wppm_bulk_change_task_priority' ) ); ?>');">
                <span><img style="margin-right:5px;" src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/priority_up_icon.svg'); ?>" alt="priority"><?php echo esc_html_e('Change Priority','taskbuilder'); ?></span>
              </div>
              <div class="wppm-popover-menu-item" onclick="wppm_bulk_delete_task('<?php echo esc_attr( wp_create_nonce( 'wppm_bulk_delete_task' ) ); ?>');">
                <span><img style="margin-right:5px;" src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/trash-alt-solid.svg'); ?>" alt="delete"><?php echo esc_html_e('Delete Tasks','taskbuilder'); ?></span>
              </div>
            </div>
        </div>
      </div>
              ...
              $checklists = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}wppm_checklist where task_id = '$task_id'" );
                if(!empty($task)){
                  ...
                  $total_checked_items=$wpdb->get_var("SELECT COUNT(Items.id) AS check_id
                    FROM {$wpdb->prefix}wppm_checklist_items AS Items
                    Left join {$wpdb->prefix}wppm_checklist checklist ON Items.checklist_id = checklist.id
                    WHERE checklist.task_id = '$task_id' AND Items.checked=1");
                  
                  if(isset($task->status)){
                    $tstatus = esc_sql($task->status);
                    $status = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wppm_task_statuses where id = '$tstatus'" );
                  }
                  if(!empty($task->priority)){
                    $tpriority = esc_sql($task->priority);
                    $priority = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wppm_task_priorities where id ='$tpriority' " );
                  }
                  if(isset($task->project)){
                    $tproj = esc_sql($task->project);
                    $project = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wppm_project where id = '$tproj'" );
                  }
                  if($wppm_task_time == 1){
                  ...
                  <tr onclick= "if(link)wppm_open_task(<?php echo esc_attr($task->id) ?>)" >
                    <td class="bulk-selector" onmouseover="link=false;" onmouseout="link=true;">
                      <div class="wppm-pl-item-selector">
                        <div class="checkbox-container">
                          <input id="wppm_bulk_select_<?php echo esc_attr($task->id) ?>" class="wppm-bulk-select" type="checkbox" onchange="wppm_bulk_task_item_select_change();" value="<?php echo esc_attr( $task->id ); ?>"/>
                          <label for="wppm_bulk_select_<?php echo esc_attr($task->id) ?>"></label>
                        </div>
                      </div>
                    </td>
 ...
 */
  function wppm_bulk_task_item_select_change() {
