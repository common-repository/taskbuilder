...
if(!empty($id)){
  ...
  $orderby_proj_sql = esc_sql(sanitize_sql_orderby( "create_time DESC" ));
  $task_comment = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}wppm_task_comment where task_id=".$id." ORDER BY ".$orderby_proj_sql);
  $task_proj = isset($task->project) ? esc_sql($task->project) :"";
  $task_id = isset($task->id) ? esc_sql($task->id):"";
}
$user_role = $wpdb->get_var( "SELECT role_id FROM {$wpdb->prefix}wppm_project_users where proj_id = '$task_proj' AND user_id = '$current_user->ID'");
if(!empty($task_proj)){
  $project_name = $wpdb->get_var( "SELECT project_name FROM {$wpdb->prefix}wppm_project where id= '$task_proj'");
}
$wppm_checklist=$wpdb->get_results( "SELECT * FROM {$wpdb->prefix}wppm_checklist where task_id='$task_id'" );
if(!empty($task->status)){
  $task_status_var = esc_sql($task->status);
  $task_status = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wppm_task_statuses where id='$task_status_var'");
}
if(!empty($task->priority)){
  $task_priority_var = esc_sql($task->priority);
  $task_priority = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wppm_task_priorities where id=".$task_priority_var);
}
if(!empty($task->users)){
    ...
    <div class="col-sm-8 wppm_body col-md-9">
      <div class="wppm_task_details_container">
        ...
        <div class="row">
          ...
          <div class="col-sm-9">
            <span class="wppm_task_description"><?php echo (isset($task_description))? wp_kses_post(wpautop($task_description)):"" ?></span>
          </div>
        </div>
        ...
        <div id="wppm_checklist_container">
          ...
          if(!empty($wppm_checklist)){
            foreach( $wppm_checklist as $key=>$list){
              ...
              <?php
              $list_id = esc_sql($list->id);
              $wppm_checklist_items = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}wppm_checklist_items where checklist_id='$list_id'");
              ?>
              <?php
        ...
        <?php 
            if(!empty($task_comment)){
              foreach($task_comment as $comment){
                $task_comment_meta = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wppm_task_comment_meta where task_id=".$id." AND comment_id=".$comment->id."");
                ...
                <div class="wppm_thread">
                  ...
                  <div class="wppm_thread_body">
                    <div class="wppm_thread_user_name">
                      ...
                      <span><?php if(!empty($comment->body) && (isset($task_comment_meta->comment_type) && $task_comment_meta->comment_type=='change_task_status')){
                      ...
                      <div class="wppm_thread_action">
                        <?php if(( (($current_user->has_cap('manage_options')) || ($wppmfunction->has_comment_permission('edit_task_comment',$id,$comment->id))) && ((empty($task_comment_meta->comment_type)) || (isset($task_comment_meta->comment_type) && $task_comment_meta->comment_type=='comment')))){ ?>
                    ...
                    <div class="wppm_thread_messege" style="color:<?php echo esc_attr($appearance_settings['comment-secondary-color']);?>">
                      ...
                      } elseif(!empty($comment->body) && (isset($task_comment_meta->comment_type) && $task_comment_meta->comment_type=='change_assign_user')) { 
                        $comment_log_body = unserialize($comment->body);
                      ...
                      } elseif(!empty($comment->body) && (isset($task_comment_meta->comment_type) && $task_comment_meta->comment_type=='change_task_priority')){ ?>
                        <span class="wppm_admin_label" style="margin-right: 10px; background-color:<?php echo isset($task_prev_prio->bg_color) ? esc_attr($task_prev_prio->bg_color):""; ?>;color:<?php echo isset($task_prev_prio->color) ? esc_attr($task_prev_prio->color): "" ?>;"><?php echo !empty($task_prev_prio->name) ? esc_attr($task_prev_prio->name) : "None" ?></span> <img src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/arrow-right.svg'); ?>" alt="arrow_right"> <span class="wppm_admin_label" style="margin-left:10px; background-color:<?php echo isset($task_new_prio->bg_color) ? esc_attr($task_new_prio->bg_color):""; ?>;color:<?php echo isset($task_new_prio->color) ? esc_attr($task_new_prio->color): "" ?>;"><?php echo !empty($task_new_prio->name) ? esc_attr($task_new_prio->name) : "None" ?></span><?php
                      } ?>
                       
                    </div>
                    <?php 
                    if(!empty($attachments)&& (isset($task_comment_meta->comment_type) && !($task_comment_meta->comment_type=='delete_task_comment'))):?> <br>
      ...
      <div class="row wppm_widget" id="wppm_raisedby_widget">
        ...
        <div class="wppm-widget-body" style="background-color:<?php echo esc_attr($appearance_settings['widget-body-bg-color'])?>">
          <div style="padding:2px 0;">
              ...
              <span class="wppm_task_user_names" style="color:<?php echo esc_attr($appearance_settings['widget-body-text-color'])?>;"><?php echo isset($user_info->display_name)?esc_html($user_info->display_name):""; ?></span>
          </div>
        </div>
      </div>
...
jQuery( document ).ready( function( jQuery ) {<?php
  ...
  if($wppm_tinymce_visibility_open_task==1){?> 
      tinymce.remove();
      tinymce.init({ 
