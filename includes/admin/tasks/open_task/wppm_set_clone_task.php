...
$prev_task_id  = isset($_POST['task_id']) ? intval(sanitize_text_field($_POST['task_id'])) : $ptask_id ;
...
if(!empty($task_comments)){
    ...
    foreach($task_comments as $task_comment){
        ...
        if(!empty($args)){
            ...
            $task_comment_type = $wpdb->get_var( "SELECT comment_type FROM {$wpdb->prefix}wppm_task_comment_meta where task_id=".$prev_task_id." AND comment_id='$task_comm_id'");
