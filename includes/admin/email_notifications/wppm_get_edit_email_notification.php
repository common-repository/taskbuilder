...
<form id="wppm_edit_email_notification_settings" method="post" action="javascript:wppm_set_edit_email_notification();">
  ...
  <div class="form-group">
    ...
    <div class="row">
      <div class="col-sm-4 proj_creator" style="margin-bottom:10px;display:flex;">
      <div style="width:25px;"><input type="checkbox" <?php echo esc_attr(in_array('project_creator',$recipients))?'checked="checked"':''?> name="wppm_en_recipients[]" value="project_creator" /></div>
      <div style="padding-top:3px;"><?php echo esc_html_e('Project Creator','taskbuilder')?></div>
    </div>
    <?php 
    if($type=='new_task' || $type=='change_task_status' || $type=='change_task_assign_users'|| $type=='new_discussion'){ 
      $tc_style='margin-bottom:10px;display:flex';
    } else{
      $tc_style='display:none';
    }?>
      <div class="col-sm-4 task_creator" style="<?php echo esc_attr($tc_style)?>">
        <div style="width:25px;"><input type="checkbox" <?php echo esc_attr(in_array('task_creator',$recipients))?'checked="checked"':''?> name="wppm_en_recipients[]" value="task_creator" /></div>
        <div style="padding-top:3px;"><?php echo esc_html_e('Task Creator','taskbuilder')?></div>
      </div>
    </div>
