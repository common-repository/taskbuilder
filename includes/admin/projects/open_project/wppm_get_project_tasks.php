...
ob_start();
?>
<form id="wppm_get_project_tasks_frm" name="wppm_get_project_tasks_frm">
        ...
        <tbody>
          ...
          if(!empty($tasks)){
              ...
              if(!empty($task->project)){
                $tproject = esc_sql($task->project);
                $project = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wppm_project where id = '$tproject'" );
              }
              if($wppm_task_time == 1){
