...
$orderby_sql = esc_sql(sanitize_sql_orderby( "load_order ASC" ));
...
if($current_user->has_cap('manage_options') || $wppm_current_user_capability == 'wppm_admin'){
	if(!empty($proj_attr )){
		$proj_attr = esc_sql($proj_attr);
		$where = " where project_name = '".$proj_attr."' Group by proj.id ORDER BY '$santitize_order_by'";
	}else{
		$where = " Group by proj.id ORDER BY '$santitize_order_by'";
	}
}else{
	$where =  " where ((FIND_IN_SET($current_user->ID,proj.users)) OR (proj.id = proj_meta.project_id AND proj_meta.meta_key='public_project' AND proj_meta.meta_value=1) OR ( proj.created_by='$current_user->ID')) $wppm_proj_attr Group by proj.id ORDER BY '$santitize_order_by'";
}
	...
	foreach($task_status as $status) {
		...
		if(!empty($search_tag)){
			...
			if($current_user->has_cap('manage_options') || $wppm_current_user_capability == 'wppm_admin'){
				...
				$where = " where $wppm_tl_filter AND $wppm_task_by_proj_filter AND (Task.task_name LIKE '$search_tag_text' OR proj.project_name LIKE '$search_tag_text' OR task_statuses.name LIKE '$search_tag_text' OR task_priorities.name LIKE '$search_tag_text' OR user.display_name LIKE '$search_tag_text') AND Task.status='$status_id' ";
			} else{
				$query = ("SELECT Task.*
					FROM {$wpdb->prefix}wppm_task AS Task
					Left join {$wpdb->prefix}wppm_project proj ON Task.project = proj.id
			...
			if($current_user->has_cap('manage_options') || $wppm_current_user_capability == 'wppm_admin'){
				...
				");
				$no_of_rows = ( "SELECT count(*) FROM ($query");
				$where = " where $wppm_tl_filter AND $wppm_task_by_proj_filter AND Task.status= '$status_id'";
			}else{
				$query = ( "SELECT Task.*
					FROM {$wpdb->prefix}wppm_task AS Task
					Left join {$wpdb->prefix}wppm_project proj ON Task.project = proj.id
							...
							foreach($tasks as $task){
								...
								if((!empty($task)) && $task->status==$status->id){
									...
									if(isset($task->priority)){
										...
										$task_priority = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}wppm_task_priorities where id = '$tpriority'" );
									}
									if($wppm_task_time == 1){
									...
									if(!empty($task->id)){
										$tid = esc_sql( $task->id);
										$checklists = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}wppm_checklist where task_id = '$tid'" );
										$total=$wpdb->get_var("SELECT COUNT(Items.id) AS check_id
											...
											WHERE checklist.task_id = '$tid'");
										$total_checked_items=$wpdb->get_var("SELECT COUNT(Items.id) AS check_id
											FROM {$wpdb->prefix}wppm_checklist_items AS Items
											Left join {$wpdb->prefix}wppm_checklist checklist ON Items.checklist_id = checklist.id
											WHERE checklist.task_id = '$tid' AND Items.checked=1");
										...
										<div class="wppm_card_body" style="background-color:<?php echo esc_attr($appearance_settings['grid-background-color'])?>!important;color:<?php echo esc_attr($appearance_settings['grid-header-text-color'])?>!important;" onclick="wppm_open_task(<?php echo esc_attr($task->id)?>)" id="wppm_draggable_card_<?php echo esc_attr($task->id)?>">
											...
											<div class="row">
												<div class="col-sm-12">
													...
													<span class="wppm_proj_name_grid_view">(<?php echo isset($project_data['project_name']) ? esc_html_e($project_data['project_name'],'taskbuilder'):"";?>)</span> 
												</div>
											</div>
											<div class="row">
