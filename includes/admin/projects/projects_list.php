...
<form id="wppm_project_list_frm">
  <div id="wppm-container">
    <div id="wppm_project_list_container" class="wppm_bootstrap">
      ...
      <div class="row">
          <div class="col-sm-12">
            <div class="wppm-bulk-actions">
                <button 
                  id="wppm-bulk-actions-btn"
                  class="wppm-button small secondary"
                  type="button"
                  style="display:none"
                  data-popover="wppm-bulk-actions">
                  <?php esc_attr_e( 'Bulk Actions ', 'taskbuilder' ); ?><img style="color:#313042" src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/wppm_chevron_down_icon.svg'); ?>" alt="chevron_down">
                </button>
                <div id="wppm-bulk-actions" class="gpopover wppm-popover-menu wppm-proj-bulk-actions">
                  <div class="wppm-popover-menu-item" onclick="wppm_bulk_change_status('<?php echo esc_attr( wp_create_nonce( 'wppm_bulk_change_status' ) ); ?>');">
                    <span><img style="margin-right:5px;" src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/gps-navigation.svg'); ?>" alt="status"><?php echo esc_html_e('Change Status','taskbuilder'); ?></span>
                  </div>
                  <div class="wppm-popover-menu-item" onclick="wppm_bulk_change_category('<?php echo esc_attr( wp_create_nonce( 'wppm_bulk_change_category' ) ); ?>');">
                    <span><img style="margin-right:5px;" src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/tags.svg'); ?>" alt="category"><?php echo esc_html_e('Change Category','taskbuilder'); ?></span>
                  </div>
                  <div class="wppm-popover-menu-item" onclick="wppm_bulk_assign_users('<?php echo esc_attr( wp_create_nonce( 'wppm_bulk_assign_users' ) ); ?>');">
                    <span><img style="margin-right:5px;" src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/users.svg'); ?>" alt="users"><?php echo esc_html_e('Assign Co-workers','taskbuilder'); ?></span>
                  </div>
                  <div class="wppm-popover-menu-item" onclick="wppm_bulk_delete_project('<?php echo esc_attr( wp_create_nonce( 'wppm_bulk_assign_users' ) ); ?>');">
                    <span><img style="margin-right:5px;" src="<?php echo esc_url( WPPM_PLUGIN_URL . 'asset/images/trash-alt-solid.svg'); ?>" alt="delete"><?php echo esc_html_e('Delete','taskbuilder'); ?></span>
                  </div>
                </div>
            </div>
          </div>
      ...
      <script>
        jQuery('#wppm-more-actions-btn, #wppm-bulk-actions-btn').gpopover({width: 200});
      </script>
      <div class="table-responsive">
        <table id="wppm_project_table" class="wppm_table table">
          <tr>
            <th style="width: 45px;"  onchange="wppm_bulk_select_change()">
              <div class="checkbox-container">
                <input id="wppm_bulk_select_option" class="wppm-bulk-selector" type="checkbox" onchange="wppm_bulk_select_change();"/>
                <label for="wppm_bulk_select_option"></label>
              </div>
            </th>
          ...
          if(!empty($wppm_fillter)){ 
            foreach( $wppm_fillter as $project ){ 
                ...
                $pid= esc_sql($project->id);
                if(isset($project->cat_id)){
                  $pcat_id = esc_sql($project->cat_id);
                  $category = $wpdb->get_var( "SELECT name FROM {$wpdb->prefix}wppm_project_categories where id='$pcat_id'" );
                }
                if(isset($project->id)){
                  if($current_user->has_cap('manage_options') || $wppm_current_user_capability == 'wppm_admin'){
                    $no_of_tasks = $wpdb->get_var("SELECT count(*) FROM {$wpdb->prefix}wppm_task WHERE project='$pid' ");
                  }else{
                  ...
                  ?>
                  <tr onclick="if(link)wppm_open_project(<?php echo esc_attr($project->id) ?>)">
                    <td class="bulk-selector" onmouseover="link=false;" onmouseout="link=true;">
                      <div class="wppm-pl-item-selector">
                        <div class="checkbox-container">
                          <input id="wppm_bulk_select_<?php echo esc_attr($project->id) ?>" class="wppm-bulk-select" type="checkbox" onchange="wppm_bulk_item_select_change();" value="<?php echo esc_attr( $project->id ); ?>"/>
                          <label for="wppm_bulk_select_<?php echo esc_attr($project->id) ?>"></label>
                        </div>
                      </div>
                    </td>
                    <td onmouseover="link=true;"><?php echo esc_html_e($project->project_name,'taskbuilder') ?></td>
                    <td onmouseover="link=true;"><span class="wppm_td_project_status" style="background-color:<?php echo esc_attr($status->bg_color);?>; color: <?php echo esc_attr($status->color); ?>"><?php echo esc_html_e($status->name,'taskbuilder') ?></span></td>
                    ...
                    <?php if($wppm_default_project_date==1){ ?>
                            ...
                            <td onmouseover="link=true;" class="wppm_table_td"><span style="<?php echo esc_attr($style); ?>"><?php echo isset($proj_end_date) ? esc_html($proj_end_date):"" ?></td></span>
                    <?php } ?>
 ...
 */
function wppm_bulk_item_select_change() {
  ...
  if(checked){
    ...
    jQuery(".wppm-bulk-selector").prop("checked", false);
  }
}
</script>
<?php
