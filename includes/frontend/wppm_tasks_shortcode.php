...
if ( ! class_exists( 'WPPM_Tasks_Shortcode' ) ) :
  final class WPPM_Tasks_Shortcode {
    public $shortcode_attr ;  
    // constructor
    public function __construct($attr) {
      global $wppmfunction;
      $this->shortcode_attr = !empty($attr)? $wppmfunction->sanitize_array($attr):array();  
      $shortcode_attr =  $this->shortcode_attr;
      $this->wppm_display_tasks();
      add_action('wp_footer', array($this,'wppm_page_inline_script' ),999999999999999999);
    }
  
    public function wppm_display_tasks(){ 
    ...
    public function wppm_page_inline_script() { ?>
      <script type="text/javascript">
        ...
        function wppm_get_task_list(page_no){
          ...
          <?php 
          if(!empty($this->shortcode_attr['project'])){ 
                ?>
                dataform.append("wppm_project_attr","<?php echo $this->shortcode_attr['project']; ?>");
          <?php } ?>
          jQuery.ajax( {
        ...
        function wppm_apply_task_filter(){
          ...
          <?php echo do_action('wppm_apply_task_filter_dataform');?>
            ...
            dataform.append("action", 'wppm_get_task_list');
            <?php if(!empty($this->shortcode_attr['project'])){ ?>
                    dataform.append("wppm_project_attr","<?php echo $this->shortcode_attr['project'] ?>");
            <?php } ?>
            jQuery.ajax( {
              url: wppm_admin.ajax_url,
        ...
        function wppm_tasks_by_select_project(){
          ...
          jQuery('#wppm_task_container').html(wppm_admin.loading_html);
          var dataform=new FormData(jQuery('#wppm_task_list_frm')[0]);
          <?php echo do_action('wppm_change_proj_tl_filter_dataform');?>
          dataform.append("action", 'wppm_get_task_list');
          dataform.append("wppm_proj_filter",wppm_proj_filter);
          jQuery.ajax( {
            url: wppm_admin.ajax_url,
            type: 'POST',
            data: dataform,
        ...
        function wppm_tasks_by_select_project_grid_view(){
          ...
          jQuery('#wppm_task_container').html(wppm_admin.loading_html);
          var dataform=new FormData(jQuery('#wppm_view_project_task')[0]);
          <?php echo do_action('wppm_change_proj_grid_view_filter_dataform');?>
          dataform.append("action", 'wppm_view_project_tasks');
          dataform.append("wppm_proj_filter",wppm_proj_filter);
          jQuery.ajax( {
            url: wppm_admin.ajax_url,
...
$GLOBALS['wppmtaskshortcode'] =  new WPPM_Tasks_Shortcode($attr);