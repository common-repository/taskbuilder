    ...
    final class WPPM_Functions {
        ...
        public static function create_project($args){
            ...
            $values = array(
                ...
                'status'=>isset($args['status'])? $args['status']:$default_proj_status,
        ...
        public static function create_task($args){
            ...
            $values = array(
                ...
                'status'=>(isset($args['status']))?$args['status']:$default_task_status,
        ...
        public static function wppm_submit_task_comment_meta($args){
            global $wpdb;
            $wpdb->insert($wpdb->prefix.'wppm_task_comment_meta',$args);
            $comment_meta_id = $wpdb->insert_id;
        ...
        public static function wppm_submit_proj_comment_meta($args){
            global $wpdb;
            $wpdb->insert($wpdb->prefix.'wppm_project_comment_meta',$args);
            $comment_meta_id = $wpdb->insert_id;
            return $comment_meta_id;
        }
        ...
        public function get_checklist_items($checklist_id){
            ...
            $checklist_id = esc_sql($checklist_id);
        ...
        public function change_task_comment($comment_id,$comment_body){
            global $wpdb;
            $comment_id = esc_sql($comment_id);
            $comment_body = esc_sql($comment_body);
            $values=array(
                'body'=>$comment_body
            );
        ...
        public function change_project_label($project_id,$project_label){
            global $wpdb;
            $project_id = esc_sql($project_id);
            $project_label = esc_sql($project_label);
            $values=array(
        ...
        public function change_project_start_date($project_id, $project_start_date){
            global $wpdb;
            $project_id = esc_sql($project_id);
            $project_start_date = esc_sql($project_start_date);
            $values=array(
        ...
        public function change_project_end_date($project_id, $project_end_date){
            global $wpdb;
            $project_id = esc_sql($project_id);
            $project_end_date = esc_sql($project_end_date);
            $values=array(
        ...
        public function change_project_description($project_id, $project_description){
            global $wpdb;
            $project_id = esc_sql($project_id);
            $project_description = esc_sql($project_description);
            $values=array(
        ...
        public function change_project_status($project_id,$status_id){
            ...
            $change_status_value= array();
            $proj_data = $wppmfunction->get_project($project_id);
            $prev_status = esc_sql($proj_data['status']);
            $project_id = esc_sql($project_id);
            ...
            $change_status_value = array('prev_status'=>"$prev_status",'new_status'=>"$status_id");
            $change_status_obj = serialize($change_status_value);
            $log_values = array('proj_id'=>$project_id,'body'=>$change_status_obj,'attachment_ids'=>"",'create_time'=>date("Y-m-d h:i:sa"),'created_by'=>$current_user->ID );
            $wpdb->insert($wpdb->prefix . 'wppm_project_comment',$log_values);
            $log_id = esc_sql($wpdb->insert_id);
            $proj_log_values = array('proj_id'=>$project_id,'comment_id'=>$log_id,'comment_type'=>'change_proj_status');
        ...
        public function get_project_creator_email($project_id){
            global $wpdb,$wppmfunction;
            $project_data = $wppmfunction->get_project($project_id,'created_by');
            $userdata = get_userdata($project_data['created_by']);
            if(!empty( $userdata)){
                $proj_creator_email[] = $userdata->user_email;
            }
            return apply_filters( 'wppm_get_project_creator_email', $proj_creator_email ,$project_id);
        }

        public function get_task_creator_email($task_id){
            global $wpdb,$wppmfunction;
            $task_data = $wppmfunction->get_task($task_id,'created_by');
            $userdata = get_userdata($task_data['created_by']);
            if(!empty( $userdata)){
                $task_creator_email[] = $userdata->user_email;
            }
            return apply_filters( 'wppm_get_task_creator_email', $task_creator_email ,$task_id);
        }
        ...
        */
        ...
        public function get_old_project_status_name($id){
            ...
            $old_project_status = esc_sql($old_project_status);
        ...
        public function get_old_task_status_name($task_id){
            ...
            $old_task_status = esc_sql($old_task_status);
